...
if (isset($_POST['register'])) {
    ...
    {die('Could not update data: ' . $connection->connect_error);}
       ...
       //get user_id for session and for future queries
        ...
        $sql="INSERT INTO Employee (employee_id, f_name, l_name, address, contact_email, category, account_status)
                VALUES  ('$user_id','$fname','$lname', '$address','$email', '$category', 'active');";
        $result = $connection->query($sql);
        if(! $result )
    {die('Could not update data: ' . $connection->connect_error);}
        //echo "Updated data successfully\n";
